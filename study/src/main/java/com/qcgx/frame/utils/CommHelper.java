...
public final class CommHelper {
	/** 日志书写对象 **/
	private static Logger logger = Logger.getLogger(CommHelper.class);
	 ...
	 * @return 属性值集<br>
	 */
	public static Map<?, ?> getLegalHashMap(Object data) {
		Map<?, ?> rtnMap = null;
		try {
			if (CommHelper.isNotNull(data)) {
				if (data instanceof Map) {
					rtnMap = (Map<?, ?>) data;
				} else {
					rtnMap = ReflectHelper.getFieldValues(data);
				}
			}
		} catch (Exception ex) {
			ex.printStackTrace(); logger.error(ex);
